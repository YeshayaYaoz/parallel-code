...
import type { Agent, Task, GitIsolationMode, AppStore, RemoteBackendConfig } from './types';
...
/**
 * Task creation for a project pointed at a remote cloud-backend instance
 * (see EditProjectDialog.tsx). Deliberately its own function rather than
 * more branches threaded through createTask() below: none of docker mode,
 * coordinator mode, steps, or worktree/direct/none git isolation apply to a
 * remote task (the remote backend does its own worktree creation on its own
 * filesystem), so keeping this separate avoids scattering `if (remote)`
 * checks through logic that a remote task never reaches.
 */
async function createRemoteBackendTask(
  opts: Pick<
    CreateTaskOptions,
    'name' | 'nameIsAutoGenerated' | 'agentDef' | 'projectId' | 'initialPrompt'
  >,
  remoteBackend: RemoteBackendConfig,
): Promise<string> {
  const { name, nameIsAutoGenerated, agentDef, projectId, initialPrompt } = opts;
  const result = await invoke<CreateTaskResult>(IPC.CreateTask, {
    name,
    remoteBackend,
  });
  if (!result.agentId) {
    throw new Error('Remote backend did not return an agentId for the new task');
  }
  ...
  const task: Task = {
    ...
    remoteBackend,
    // Delivered once the agent's output shows it's ready — same mechanism
    // as a local task, since output/input are transparently bridged too.
    initialPrompt: initialPrompt ?? undefined,
    savedInitialPrompt: initialPrompt ?? undefined,
  };

  const agent = createAgentRecord({ id: agentId, taskId: result.id, def: agentDef });
...
export async function createTask(opts: CreateTaskOptions): Promise<string> {
  ...
  const remoteBackend = getProject(projectId)?.remoteBackend;
  if (remoteBackend) {
    return createRemoteBackendTask(
      { name, nameIsAutoGenerated, agentDef, projectId, initialPrompt },
      remoteBackend,
    );
  }
...
export async function closeTask(taskId: string): Promise<void> {
  ...
  try {
    ...
    if (task.remoteBackend) {
      // Remote cleanup (kill the agent + remove its worktree/branch) lives
      // entirely on the remote backend — see deleteRemoteTask in
      // electron/ipc/remote-agent-bridge.ts.
      await invoke(IPC.DeleteTask, { taskId, agentIds: [], remoteBackend: task.remoteBackend });
    } else if (task.gitIsolation === 'worktree' && !task.externalWorktree) {
