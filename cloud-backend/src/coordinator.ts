...
export class Coordinator {
  ...
  private notifier: ((channel: string, data: unknown) => void) | null = null;
  ...
  setNotifier(notify: (channel: string, data: unknown) => void): void {
  ...
  async createTask(opts: {
    ...
    try {
      ...
      // Docker-in-Docker sub-task isolation (each sub-task getting its own
      // `docker run` container) is not supported here — the cloud container
      // this service runs in is already the isolation boundary, so
      // dockerContainerName stays null in practice and every sub-task is a
      // plain PTY in this container, same as the coordinator's own agent.
      spawnAgent({
        taskId: task.id,
        agentId,
  ...
  removeCoordinatedTask(taskId: string): void {
    ...
    // No Docker sub-task cleanup needed here — this service never spawns
    // sub-tasks in their own nested container (see spawnAgent call above).
  ...
  private async cleanupTask(taskId: string): Promise<void> {
    ...
    // Kill the agent. Just a plain PTY kill — no nested sub-task container to
    // stop, since this service is never itself the Docker coordinator mode.
    try {
      killAgent(task.agentId);
    } catch {
      /* already dead */
