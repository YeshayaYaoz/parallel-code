...
describe('askAboutCodeOpenai', () => {
  ...
  it('throws if prompt exceeds max length', () => {
    ...
    expect(() =>
      askAboutCodeOpenai(win, { requestId: 'r1', channelId: 'ch1', prompt: longPrompt }),
  ...
  it('throws if no API key is set', () => {
    ...
    const { win } = makeMockWin();
    expect(() =>
      askAboutCodeOpenai(win, { requestId: 'r1b', channelId: 'ch1b', prompt: 'hi' }),
  ...
  it('sends chunk messages for each SSE delta', async () => {
    ...
    askAboutCodeOpenai(win, { requestId: 'r2', channelId: 'ch2', prompt: 'Explain this code' });
  ...
  it('sends error message on non-ok HTTP response', async () => {
    ...
    askAboutCodeOpenai(win, { requestId: 'r3', channelId: 'ch3', prompt: 'What is this?' });
    await waitForDone(messages);
  ...
  it('sends error message when fetch rejects', async () => {
    ...
    askAboutCodeOpenai(win, { requestId: 'r4', channelId: 'ch4', prompt: 'Explain' });
    await waitForDone(messages);
  ...
  it('sends correct Authorization header with Bearer token', async () => {
    ...
    setOpenaiApiKey('my-secret-key');
    askAboutCodeOpenai(win, { requestId: 'r5', channelId: 'ch5', prompt: 'Explain' });
    await waitForDone(messages);
  ...
  it('uses the configured OpenAI model', async () => {
    ...
    askAboutCodeOpenai(win, { requestId: 'r6', channelId: 'ch6', prompt: 'Test' });
    await waitForDone(messages);
  ...
  it('uses streaming mode', async () => {
    ...
    askAboutCodeOpenai(win, { requestId: 'r8', channelId: 'ch8', prompt: 'Test' });
    await waitForDone(messages);
  ...
  it('does not send to destroyed window', async () => {
    ...
    askAboutCodeOpenai(win, { requestId: 'r9', channelId: 'ch9', prompt: 'Test' });
    await new Promise((r) => setTimeout(r, 100));
    expect(messages).toHaveLength(0);
  });
});

describe('cancelAskAboutCodeOpenai', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    setOpenaiApiKey('test-key');
  });
  ...
  it('cancels a pending request without sending an error message', async () => {
    ...
    askAboutCodeOpenai(win, { requestId: 'cancel-1', channelId: 'ch-cancel', prompt: 'Test' });
    await new Promise((r) => setTimeout(r, 20));

    cancelAskAboutCodeOpenai('cancel-1');
    await waitForDone(messages);
  ...
  it('is a no-op for unknown requestId', () => {
    expect(() => cancelAskAboutCodeOpenai('unknown-id')).not.toThrow();
  });
});
