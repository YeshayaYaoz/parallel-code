...
interface DeepseekAskCodeRequest {
  requestId: string;
  channelId: string;
  prompt: string;
}
...
/** Main-process storage for the DeepSeek API key. Never sent back to the renderer. */
let storedApiKey = '';
...
export function askAboutCodeDeepseek(win: BrowserWindow, args: DeepseekAskCodeRequest): void {
  const { requestId, channelId, prompt } = args;
  const apiKey = storedApiKey;

  if (!apiKey) {
    throw new Error('DeepSeek API key is not set. Please configure it in Settings.');
  }

  assertPromptWithinLimit(prompt);
  assertCanStart(activeRequests, requestId);

  cancelAskAboutCodeDeepseek(requestId);
    ...
    .then(async (res) => {
      ...
      const onAbort = () => {
        ...
        reader.cancel().catch((err) => {
          logDebug('askCode.deepseek', 'reader.cancel rejected', { err: String(err) });
        });
      };
...
export function cancelAskAboutCodeDeepseek(requestId: string): void {
  activeRequests.cancel(requestId);
}

export function isDeepseekRequestActive(requestId: string): boolean {
